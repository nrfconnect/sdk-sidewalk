...
#define API_FUNCTIONS                                                                              \
	...
	X(sid_put_msg, struct sid_handle *, handle, const struct sid_msg *, msg,                   \
	  struct sid_msg_desc *, msg_desc)                                                         \
	X(sid_set_factory_reset, struct sid_handle *, handle)                                      \
	X(sid_get_mtu, struct sid_handle *, handle, enum sid_link_type, link_type, size_t *, mtu)  \
	...
	X(sid_get_time, struct sid_handle *, handle, enum sid_time_format, format,                 \
	  struct sid_timespec *, curr_time)                                                        \
	X(sid_set_msg_dest_id, struct sid_handle *, handle, uint32_t, id)                          \
	X(sid_option, struct sid_handle *, handle, enum sid_option, option, void *, data, size_t,  \
	  len)
...
#define X(name, ...)                                                                               \
	sid_error_t name##_delegated(WINDOW_ITERATOR_SIZE_2(ARGUMENT_LIST, (, ), __VA_ARGS__));
API_FUNCTIONS
#undef X
#undef ARGUMENT_LIST
