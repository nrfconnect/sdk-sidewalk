...
* Software-based MCUboot downgrade protection.
  Enable overwrite-only mode (``SB_CONFIG_MCUBOOT_MODE_OVERWRITE_ONLY=y``) and downgrade checks (MCUboot image Kconfig option ``CONFIG_MCUBOOT_DOWNGRADE_PREVENTION=y``).
