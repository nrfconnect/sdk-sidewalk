 ...
 *
 */
enum sid_link_status {
    /** Used to indicate the sidewalk library is connected to a Sidewalk gateway */
    SID_STATUS_LINK_UP = 0,

    /** Used when the Sidewalk library is disconnected from a Sidewalk gateway */
    SID_STATUS_LINK_DOWN = 1,
};
...
/** The physical stats of received message */
struct sid_msg_phy_stats {
    int8_t rssi;
    int8_t snr;
};
 ...
 */
struct sid_msg_desc {
    /** The link type this message is to be sent or was received from */
    enum sid_link_type link_type;
    /** The message type */
    enum sid_msg_type type;
    struct sid_msg_phy_stats phy_stats;
...
/**
 * The set of options settable by sid_set_option().
 */
enum sid_option {
 ...
 * Link status will be changed to LINK_DOWN and time sync status is cached after sid_stop() is
 * called.
 *
 ...
 * @param[in] data   A pointer to an array of data associated with the option
 * @param[in] len    The size of the data array
 *
 * @returns #SID_ERROR_NONE on success.
 */
sid_error_t sid_option(struct sid_handle *handle, enum sid_option option, const void *data, size_t len);
