...
void test_sid_pal_storage_no_init(void)
{
	...
	TEST_ASSERT_EQUAL(SID_ERROR_STORAGE_READ_FAIL,
			  sid_pal_storage_kv_record_get(GROUP_ID_TEST_OK, TEST_KEY_1,
							&test_read_data, sizeof(test_read_data)));
	TEST_ASSERT_EQUAL(SID_ERROR_STORAGE_READ_FAIL,
...
void test_sid_pal_storage_incorrect_group_id(void)
{
	...
	TEST_ASSERT_EQUAL(SID_ERROR_PARAM_OUT_OF_RANGE,
			  sid_pal_storage_kv_record_get(GROUP_ID_TEST_NOK, TEST_KEY_1,
							&test_read_data, sizeof(test_read_data)));
	TEST_ASSERT_EQUAL(SID_ERROR_PARAM_OUT_OF_RANGE,
...
void test_sid_pal_storage_write_get(void)
{
	...
	// Write many data
	for (test_keys_t tk = 0; tk < TEST_KEY_END; tk++) {
		TEST_ASSERT_EQUAL(SID_ERROR_NONE, sid_pal_storage_kv_record_set(
							  GROUP_ID_TEST_OK, tk, &test_data[tk],
							  sizeof(test_data[tk])));
	}
	...
	for (test_keys_t tk = 0; tk < TEST_KEY_END; tk++) {
		...
		TEST_ASSERT_EQUAL(SID_ERROR_NONE, sid_pal_storage_kv_record_get(
	...
	TEST_ASSERT_EQUAL(SID_ERROR_NONE, sid_pal_storage_kv_record_set(
						  GROUP_ID_TEST_OK, test_key_x, test_data_buffer,
	...
	TEST_ASSERT_EQUAL(SID_ERROR_OUT_OF_RESOURCES,
			  sid_pal_storage_kv_record_set(GROUP_ID_TEST_OK, test_key_x,
							test_data_buffer, sector_size));
	...
	for (test_keys_t tk = 0; tk < TEST_KEY_END; tk++) {
		if (TEST_KEY_5 == tk) {
			TEST_ASSERT_EQUAL(SID_ERROR_NONE, sid_pal_storage_kv_record_get_len(
			...
			TEST_ASSERT_EQUAL_HEX8_ARRAY(update_val_read, update_val,
						     sizeof(update_val_read));
		} else {
			TEST_ASSERT_EQUAL(SID_ERROR_NONE, sid_pal_storage_kv_record_get_len(
								  GROUP_ID_TEST_OK, tk, &len));
...
void test_sid_pal_storage_erase_all_2(void)
{
	...
	TEST_ASSERT_EQUAL(sizeof(test_data_buffer), len);
	TEST_ASSERT_EQUAL(SID_ERROR_NONE,
			  sid_pal_storage_kv_record_get(GROUP_ID_TEST_OK, test_key_x,
							&test_read_data, sizeof(test_read_data)));
