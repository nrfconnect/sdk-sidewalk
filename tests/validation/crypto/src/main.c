...
static uint8_t aes_ctr_plaintext[32] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
					 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F,
					 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
					 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F };
static uint8_t aes_ctr_key[16] = { 0x7E, 0x24, 0x06, 0x78, 0x17, 0xFA, 0xE0, 0xD7,
				   0x43, 0xD6, 0xCE, 0x1F, 0x32, 0x53, 0x91, 0x63 };
static uint8_t aes_ctr_iv[16] = { 0x00, 0x6C, 0xB6, 0xDB, 0xC0, 0x54, 0x3B, 0x59,
				  0xDA, 0x48, 0xD9, 0x0B, 0x00, 0x00, 0x00, 0x01 };
static uint8_t aes_ctr_ciphertext[32] = { 0x51, 0x04, 0xA1, 0x06, 0x16, 0x8A, 0x72, 0xD9,
					  0x79, 0x0D, 0x41, 0xEE, 0x8E, 0xDA, 0xD3, 0x88,
					  0xEB, 0x2E, 0x1E, 0xFC, 0x46, 0xDA, 0x57, 0xC8,
					  0xFC, 0xE6, 0x30, 0xDF, 0x91, 0x41, 0xBE, 0x28 };
static uint8_t aes_ctr_invalid_key[16] = { 0x7E, 0x24, 0x06, 0x78, 0x17, 0xFA, 0xE0, 0xD7,
					   0x43, 0xD6, 0xCE, 0x1F, 0x32, 0x53, 0x91, 0xFF };
static uint8_t aes_ctr_invalid_iv[16] = { 0x00, 0x6C, 0xB6, 0xDB, 0xC0, 0x54, 0x3B, 0x59,
					  0xDA, 0x48, 0xD9, 0x0B, 0x00, 0x00, 0x00, 0xFF };
static uint8_t aes_ctr_invalid_ciphertext[32] = { 0x51, 0x04, 0xA1, 0x06, 0x16, 0x8A, 0x72, 0xD9,
						  0x79, 0x0D, 0x41, 0xEE, 0x8E, 0xDA, 0xD3, 0x88,
						  0xEB, 0x2E, 0x1E, 0xFC, 0x46, 0xDA, 0x57, 0xC8,
...
static void test_ecc_key_gen_invalid_param(sid_pal_ecc_algo_t algo, size_t prk_sz, size_t puk_sz)
{
	...
	zassert_not_equal(ret, SID_ERROR_NONE);
...
static void ecc_ecdh_positive(sid_pal_ecc_algo_t algo, size_t prk_sz, size_t puk_sz)
{
	...
	ret = sid_pal_crypto_ecc_ecdh(&params);
	zassert_equal(SID_ERROR_NONE, ret);
	zassert_equal(0, memcmp(shared_secret, shared_secret_remote, params.shared_secret_sz));
	...
	zassert_equal(0, memcmp(shared_secret, shared_secret_remote, params.shared_secret_sz));
	zassert_equal(0,
		      memcmp(get_ecc_shared_secret(algo), shared_secret, params.shared_secret_sz));
}
...
static void ecc_ecdh_negative(sid_pal_ecc_algo_t algo, size_t prk_sz, size_t puk_sz)
{
	...
	ret = sid_pal_crypto_ecc_ecdh(&params);
	zassert_equal(SID_ERROR_NONE, ret);
	zassert_not_equal(0, memcmp(shared_secret, shared_secret_remote, params.shared_secret_sz));
}
...
ZTEST(crypto, test_random_number_gen_positive)
{
	...
	zassert_equal(SID_ERROR_NONE, ret);
	zassert_not_equal(0, memcmp(zero_buffer, buf1, sizeof(buf1)));

	ret = sid_pal_crypto_rand(buf2, sizeof(buf2));
	zassert_equal(SID_ERROR_NONE, ret);
	zassert_not_equal(0, memcmp(zero_buffer, buf2, sizeof(buf2)));

	zassert_not_equal(0, memcmp(buf1, buf2, sizeof(buf1)));
}

ZTEST(crypto, test_random_number_gen_invalid_param)
{
...
ZTEST(crypto, test_hash_positive)
{
	sid_pal_hash_params_t hash_params;
	uint8_t out_buf[64];
...
ZTEST(crypto, test_hash_invalid_param_sha512)
{
	test_hash_invalid_param(SID_PAL_HASH_SHA512);
}

ZTEST(crypto, test_hmac_positive)
{
	uint8_t out_buf[64];
	sid_pal_hmac_params_t hmac_params;
...
ZTEST(crypto, test_hmac_invalid_param_sha512)
{
	test_hmac_invalid_param(SID_PAL_HASH_SHA256);
}

ZTEST(crypto, test_hmac_negative)
{
	uint8_t out_buf[64];
	sid_pal_hmac_params_t hmac_params = {
...
ZTEST(crypto, test_cmac_positive)
{
	...
	zassert_equal(SID_ERROR_NONE, ret);
	zassert_equal(0, memcmp(out_buf, aes_mac, aes_params.out_size));
}

ZTEST(crypto, test_cmac_invalid_param)
{
	uint8_t out_buf[16];
	sid_pal_aes_params_t aes_params = {
...
ZTEST(crypto, test_cmac_invalid_input_combo)
{
	uint8_t out_buf[64];
	sid_pal_aes_params_t aes_params = {
		.algo = SID_PAL_AES_CMAC_128,
...
ZTEST(crypto, test_cmac_negative)
{
	uint8_t out_buf[64];
	sid_pal_aes_params_t aes_params = {
		.algo = SID_PAL_AES_CMAC_128,
	...
	zassert_equal(SID_ERROR_NONE, ret);
	zassert_not_equal(0, memcmp(out_buf, aes_mac, aes_params.out_size));
...
ZTEST(crypto, test_gcm_positive)
{
	...
	zassert_equal(SID_ERROR_NONE, ret);
	zassert_equal(0, memcmp(out_buf, aes_gcm_ciphertext, aead_params.out_size));
	zassert_equal(0, memcmp(out_mac_buf, aes_gcm_mac, 16));
	...
	ret = sid_pal_crypto_aead_crypt(&aead_params);
	zassert_equal(SID_ERROR_NONE, ret);
	zassert_equal(0, memcmp(out_buf, aes_gcm_plaintext, aead_params.out_size));
}

ZTEST(crypto, test_gcm_invalid_params_encrypt)
{
	test_gcm_invalid_params(SID_PAL_CRYPTO_ENCRYPT);
}

ZTEST(crypto, test_gcm_invalid_params_decrypt)
{
	test_gcm_invalid_params(SID_PAL_CRYPTO_DECRYPT);
}

ZTEST(crypto, test_gcm_negative)
{
	uint8_t out_buf[64];
	uint8_t out_mac_buf[16];
	...
	zassert_equal(SID_ERROR_NONE, ret);
	zassert_not_equal(0, memcmp(out_buf, aes_gcm_ciphertext, aead_params.out_size));
	zassert_not_equal(0, memcmp(out_mac_buf, aes_gcm_mac, aead_params.mac_size));
...
ZTEST(crypto, test_ctr_positive)
{
	sid_pal_aes_params_t aes_params = { 0 };
	uint8_t out_buf[64];
	...
	zassert_equal(SID_ERROR_NONE, ret);
	zassert_equal(0, memcmp(out_buf, aes_ctr_ciphertext, aes_params.out_size));
	...
	ret = sid_pal_crypto_aes_crypt(&aes_params);
	zassert_equal(SID_ERROR_NONE, ret);
	zassert_equal(0, memcmp(out_buf, aes_ctr_plaintext, aes_params.out_size));
}

ZTEST(crypto, test_ctr_invalid_params_encrypt)
{
	test_ctr_invalid_params(SID_PAL_CRYPTO_ENCRYPT);
}

ZTEST(crypto, test_ctr_invalid_params_decrypt)
{
	test_ctr_invalid_params(SID_PAL_CRYPTO_DECRYPT);
}

ZTEST(crypto, test_ctr_invalid_input_combo)
{
	uint8_t out_buf[64];
	sid_pal_aes_params_t aes_params = {
		.algo = SID_PAL_AES_CTR_128,
...
ZTEST(crypto, test_ctr_negative)
{
	uint8_t out_buf[64];
	sid_pal_aes_params_t aes_params = {
		.algo = SID_PAL_AES_CTR_128,
	...
	zassert_equal(SID_ERROR_NONE, ret);
	zassert_not_equal(0, memcmp(out_buf, aes_ctr_ciphertext, aes_params.out_size));
	...
	ret = sid_pal_crypto_aes_crypt(&aes_params);
	zassert_equal(SID_ERROR_NONE, ret);
	zassert_not_equal(0, memcmp(out_buf, aes_ctr_plaintext, aes_params.out_size));
...
ZTEST(crypto, test_ecc_key_gen_positive)
{
	uint8_t prk[64], puk[64];

	sid_error_t ret = ecc_key_gen_positive(SID_PAL_ECDH_CURVE25519, prk, 32, puk, 32);

	zassert_equal(ret, SID_ERROR_NONE);
	zassert_not_equal(0, memcmp(prk, zero_buffer, 32));
	zassert_not_equal(0, memcmp(puk, zero_buffer, 32));
	...
	zassert_equal(ret, SID_ERROR_NONE);
	zassert_not_equal(0, memcmp(prk, zero_buffer, 32));
	zassert_not_equal(0, memcmp(puk, zero_buffer, 64));
}

ZTEST(crypto, test_ecc_key_gen_invalid_param_ecdh_curve25519)
{
	test_ecc_key_gen_invalid_param(SID_PAL_ECDH_CURVE25519, 32, 32);
}

ZTEST(crypto, test_ecc_key_gen_invalid_param_eddsa_ed25519)
{
	test_ecc_key_gen_invalid_param(SID_PAL_EDDSA_ED25519, 32, 32);
}

ZTEST(crypto, test_ecc_key_gen_invalid_param_ecdsa_secp256r1)
{
	test_ecc_key_gen_invalid_param(SID_PAL_ECDSA_SECP256R1, 32, 64);
}

ZTEST(crypto, test_ecc_ecdh_curve25519_positive)
{
	ecc_ecdh_positive(SID_PAL_ECDH_CURVE25519, 32, 32);
}

ZTEST(crypto, test_ecc_ecdh_secp256r1_positive)
{
	ecc_ecdh_positive(SID_PAL_ECDH_SECP256R1, 32, 64);
}

ZTEST(crypto, test_ecc_ecdh_invalid_param_ecdh_curve25519)
{
	test_ecc_ecdh_invalid_param(SID_PAL_ECDH_CURVE25519, 32, 32);
}

ZTEST(crypto, test_ecc_ecdh_invalid_param_ecdh_secp256r1)
{
	test_ecc_ecdh_invalid_param(SID_PAL_ECDH_SECP256R1, 32, 64);
}

ZTEST(crypto, test_ecc_ecdh_curve25519_negative)
{
	ecc_ecdh_negative(SID_PAL_ECDH_CURVE25519, 32, 32);
}

ZTEST(crypto, test_ecc_ecdh_secp256r1_negative)
{
	ecc_ecdh_negative(SID_PAL_ECDH_SECP256R1, 32, 64);
}

ZTEST(crypto, test_ecc_dsa_ed25519_positive)
{
	ecc_dsa_positive(SID_PAL_EDDSA_ED25519, 32, 32);
}

ZTEST(crypto, test_ecc_dsa_secp256r1_positive)
{
	ecc_dsa_positive(SID_PAL_ECDSA_SECP256R1, 32, 64);
}

ZTEST(crypto, test_ecc_dsa_invalid_param_eddsa_ed25519)
{
	test_ecc_dsa_invalid_params(SID_PAL_EDDSA_ED25519, 32, 32);
}

ZTEST(crypto, test_ecc_dsa_invalid_param_ecdsa_secp256r1)
{
	test_ecc_dsa_invalid_params(SID_PAL_ECDSA_SECP256R1, 32, 64);
}

ZTEST(crypto, test_ecc_dsa_ed25519_negative)
{
	ecc_dsa_negative(SID_PAL_EDDSA_ED25519, 32, 32);
}

ZTEST(crypto, test_ecc_dsa_secp256r1_negative)
{
	ecc_dsa_negative(SID_PAL_ECDSA_SECP256R1, 32, 64);
}

ZTEST_SUITE(crypto, NULL, NULL, setUp, tearDown, NULL);
