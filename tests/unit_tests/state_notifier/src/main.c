...
ZTEST(happy_case, test_one_receiver){
	...
	zassert_mem_equal(&handler1_saved.arguments[0].state, &expected_state, sizeof(struct notifier_state),
...
ZTEST(happy_case, test_two_receivers){
	...
	zassert_mem_equal(&handler2_saved.arguments[0].state, &expected_state, sizeof(struct notifier_state),
			  "Application state invalid");
}
...
ZTEST(utils, test_enumerate_differences_1_difference){
	...
	zassert_equal(APPLICATION_STATE_s2, enumerate_mock_history.args[0].state_id, "Invalid state change called");
	zassert_equal(1, enumerate_mock_history.args[0].value, "Invalid value on change enumerate element");
}
...
ZTEST(utils, test_enumerate_states){
	...
	zassert_equal(APPLICATION_STATE_s1, enumerate_mock_history.args[0].state_id, "Invalid state change called");
	zassert_equal(1, enumerate_mock_history.args[0].value, "Invalid value on change enumerate element");

	zassert_equal(APPLICATION_STATE_s2, enumerate_mock_history.args[1].state_id, "Invalid state change called");
	zassert_equal(0, enumerate_mock_history.args[1].value, "Invalid value on change enumerate element");

	zassert_equal(APPLICATION_STATE_s3, enumerate_mock_history.args[2].state_id, "Invalid state change called");
	zassert_equal(0, enumerate_mock_history.args[2].value, "Invalid value on change enumerate element");
