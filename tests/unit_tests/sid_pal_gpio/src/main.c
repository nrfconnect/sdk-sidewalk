...
void test_sid_pal_gpio_set_direction_fail(void)
{
	TEST_ASSERT_EQUAL(SID_ERROR_INVALID_ARGS,
			  sid_pal_gpio_set_direction(GPIO_NUMBER, INVALID_DIRECTION_VALUE));

	__cmock_sid_gpio_utils_port_pin_get_IgnoreAndReturn(-ENOTSUP);
	TEST_ASSERT_EQUAL(SID_ERROR_NOSUPPORT,
			  sid_pal_gpio_set_direction(INVALID_GPIO, SID_PAL_GPIO_DIRECTION_INPUT));
...
void test_sid_pal_gpio_set_direction_pass(void)
{
	...
	TEST_ASSERT_EQUAL(SID_ERROR_NONE,
			  sid_pal_gpio_set_direction(GPIO_NUMBER, SID_PAL_GPIO_DIRECTION_INPUT));
	TEST_ASSERT_EQUAL(SID_ERROR_NONE,
			  sid_pal_gpio_set_direction(GPIO_NUMBER, SID_PAL_GPIO_DIRECTION_OUTPUT));
}

void test_sid_pal_gpio_input_mode_fail(void)
{
	TEST_ASSERT_EQUAL(SID_ERROR_INVALID_ARGS,
			  sid_pal_gpio_input_mode(GPIO_NUMBER, INVALID_INPUT_MODE_VALUE));

	__cmock_sid_gpio_utils_port_pin_get_IgnoreAndReturn(-ENOTSUP);
	TEST_ASSERT_EQUAL(SID_ERROR_NOSUPPORT,
			  sid_pal_gpio_input_mode(INVALID_GPIO, SID_PAL_GPIO_INPUT_CONNECT));
	...
	__cmock_sid_gpio_utils_port_pin_get_IgnoreAndReturn(E_OK);
	TEST_ASSERT_EQUAL(SID_ERROR_INVALID_ARGS,
			  sid_pal_gpio_input_mode(GPIO_NUMBER, SID_PAL_GPIO_INPUT_CONNECT));
	...
	TEST_ASSERT_EQUAL(SID_ERROR_IO_ERROR,
			  sid_pal_gpio_input_mode(GPIO_NUMBER, SID_PAL_GPIO_INPUT_CONNECT));

	__cmock_sid_gpio_utils_port_pin_get_IgnoreAndReturn(E_OK);
	__cmock_gpio_pin_configure_IgnoreAndReturn(-EIO);
	TEST_ASSERT_EQUAL(SID_ERROR_IO_ERROR,
			  sid_pal_gpio_input_mode(GPIO_NUMBER, SID_PAL_GPIO_INPUT_DISCONNECT));
}
...
{
	TEST_ASSERT_EQUAL(SID_ERROR_INVALID_ARGS,
			  sid_pal_gpio_output_mode(GPIO_NUMBER, INVALID_INPUT_MODE_VALUE));

	__cmock_sid_gpio_utils_port_pin_get_IgnoreAndReturn(-ENOTSUP);
	TEST_ASSERT_EQUAL(SID_ERROR_NOSUPPORT,
			  sid_pal_gpio_output_mode(INVALID_GPIO, SID_PAL_GPIO_OUTPUT_PUSH_PULL));
	...
	__cmock_sid_gpio_utils_port_pin_get_IgnoreAndReturn(E_OK);
	TEST_ASSERT_EQUAL(SID_ERROR_INVALID_ARGS,
			  sid_pal_gpio_output_mode(GPIO_NUMBER, SID_PAL_GPIO_OUTPUT_PUSH_PULL));
	...
	__cmock_gpio_pin_configure_IgnoreAndReturn(-EIO);
	TEST_ASSERT_EQUAL(SID_ERROR_IO_ERROR,
			  sid_pal_gpio_output_mode(GPIO_NUMBER, SID_PAL_GPIO_OUTPUT_OPEN_DRAIN));
...
void test_sid_pal_gpio_pull_mode_fail(void)
{
	__cmock_sid_gpio_utils_port_pin_get_IgnoreAndReturn(-ENOTSUP);
	TEST_ASSERT_EQUAL(SID_ERROR_NOSUPPORT,
			  sid_pal_gpio_pull_mode(INVALID_GPIO, SID_PAL_GPIO_PULL_NONE));

	__cmock_sid_gpio_utils_port_pin_get_IgnoreAndReturn(E_OK);
	TEST_ASSERT_EQUAL(SID_ERROR_INVALID_ARGS,
			  sid_pal_gpio_pull_mode(GPIO_NUMBER, INVALID_PULL_MODE_VALUE));

	__cmock_sid_gpio_utils_port_pin_get_IgnoreAndReturn(E_OK);
	__cmock_gpio_pin_configure_IgnoreAndReturn(-EIO);
	TEST_ASSERT_EQUAL(SID_ERROR_IO_ERROR,
			  sid_pal_gpio_pull_mode(GPIO_NUMBER, SID_PAL_GPIO_PULL_NONE));
}
...
void test_sid_pal_gpio_pull_mode_pass(void)
{
	...
	TEST_ASSERT_EQUAL(SID_ERROR_NONE,
			  sid_pal_gpio_pull_mode(GPIO_NUMBER, SID_PAL_GPIO_PULL_UP));

	TEST_ASSERT_EQUAL(SID_ERROR_NONE,
			  sid_pal_gpio_pull_mode(GPIO_NUMBER, SID_PAL_GPIO_PULL_DOWN));
}
...
void test_sid_pal_gpio_set_irq_fail(void)
{
	...
	TEST_ASSERT_EQUAL(SID_ERROR_INVALID_ARGS,
			  sid_pal_gpio_set_irq(GPIO_NUMBER, INVALID_IRQ_TRIGGER_VALUE,
	...
	TEST_ASSERT_EQUAL(SID_ERROR_NOSUPPORT,
			  sid_pal_gpio_set_irq(INVALID_GPIO, SID_PAL_GPIO_IRQ_TRIGGER_RISING,
					       gpio_irq_handler, (void *)&handle_argument));
...
void test_sid_pal_gpio_set_irq_pass(void)
{
	sid_pal_gpio_irq_trigger_t irq_trigger_list[] = {
		SID_PAL_GPIO_IRQ_TRIGGER_NONE,	  SID_PAL_GPIO_IRQ_TRIGGER_RISING,
		SID_PAL_GPIO_IRQ_TRIGGER_FALLING, SID_PAL_GPIO_IRQ_TRIGGER_EDGE,
		SID_PAL_GPIO_IRQ_TRIGGER_LOW,	  SID_PAL_GPIO_IRQ_TRIGGER_HIGH
	};
	const uint8_t handle_argument = 5;

	for (sid_pal_gpio_irq_trigger_t trigger = SID_PAL_GPIO_IRQ_TRIGGER_NONE;
	     trigger < ARRAY_SIZE(irq_trigger_list); trigger++) {
		...
		TEST_ASSERT_EQUAL(SID_ERROR_NONE,
				  sid_pal_gpio_set_irq(GPIO_NUMBER, trigger, gpio_irq_handler,
						       (void *)&handle_argument));
	}
}
