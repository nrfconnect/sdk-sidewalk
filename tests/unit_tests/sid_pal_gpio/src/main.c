...
#include <mock_sid_gpio_irq.h>
#include <mock_sid_gpio_irq_handler.h>
#include <unity.h>
...
void test_sid_pal_gpio_read_fail(void)
{
	...
	__wrap_sid_gpio_utils_gpio_read_IgnoreAndReturn(-ENOTSUP);
...
void test_sid_pal_gpio_set_direction_fail(void)
{
	...
	__wrap_gpio_pin_configure_IgnoreAndReturn(-EINVAL);
...
{
	__wrap_sid_gpio_utils_port_pin_get_IgnoreAndReturn(E_OK);
	__wrap_gpio_pin_configure_IgnoreAndReturn(E_OK);
...
void test_sid_pal_gpio_input_mode_fail(void)
{
	...
	TEST_ASSERT_EQUAL(SID_ERROR_NONE, sid_pal_gpio_set_direction(GPIO_NUMBER, SID_PAL_GPIO_DIRECTION_INPUT));
	__wrap_sid_gpio_utils_port_pin_get_IgnoreAndReturn(E_OK);
	__wrap_gpio_pin_configure_IgnoreAndReturn(-EIO);
...
void test_sid_pal_gpio_set_irq_fail(void)
{
	...
	__wrap_sid_gpio_irq_handler_set_Ignore();
	__wrap_sid_gpio_irq_configure_IgnoreAndReturn(-ENOTSUP);
	TEST_ASSERT_EQUAL(SID_ERROR_NOSUPPORT, sid_pal_gpio_set_irq(INVALID_GPIO, SID_PAL_GPIO_IRQ_TRIGGER_RISING,
								    gpio_irq_handler, (void *)&handle_argument));
	__wrap_sid_gpio_irq_handler_set_StopIgnore();
}
...
void test_sid_pal_gpio_set_irq_pass(void)
{
	...
	for (sid_pal_gpio_irq_trigger_t trigger = SID_PAL_GPIO_IRQ_TRIGGER_NONE; trigger < ARRAY_SIZE(irq_trigger_list); trigger++) {
		__wrap_sid_gpio_irq_configure_IgnoreAndReturn(E_OK);
		if (SID_PAL_GPIO_IRQ_TRIGGER_NONE == trigger) {
			__wrap_sid_gpio_irq_handler_set_Expect(GPIO_NUMBER, NULL, NULL);
		} else {
			__wrap_sid_gpio_irq_handler_set_Expect(GPIO_NUMBER, gpio_irq_handler, (void *)&handle_argument);
		}
...
{
	__wrap_sid_gpio_irq_set_IgnoreAndReturn(-ENOTSUP);
...
{
	__wrap_sid_gpio_irq_set_IgnoreAndReturn(E_OK);
