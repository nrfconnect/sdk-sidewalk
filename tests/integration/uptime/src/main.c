...
ZTEST(uptime, test_sid_pal_uptime_get)
{
	...
	zassert_true(sid_time.tv_sec >= time_befre.tv_sec, "sid uptime before lower bound");
	zassert_true((sid_time.tv_nsec >= time_befre.tv_nsec) ||
			     (sid_time.tv_sec > time_befre.tv_sec),
		     "sid uptime before lower bound (nsec)");
	zassert_true(sid_time.tv_sec <= time_after.tv_sec, "sid uptime after upper bound");
	zassert_true((sid_time.tv_nsec <= time_after.tv_nsec) ||
			     (sid_time.tv_sec < time_after.tv_sec),
		     "sid uptime after upper bound (nsec)");
}
