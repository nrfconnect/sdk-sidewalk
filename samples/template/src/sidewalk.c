...
static const struct smf_state sid_states[] = {
	[STATE_SIDEWALK] = SMF_CREATE_STATE(state_sidewalk_entry, state_sidewalk_run, NULL, NULL),
...
#endif /* CONFIG_SIDEWALK_AUTO_CONN_REQ */
		break;
	case SID_EVENT_OPTION_SET:
		sidewalk_option_t *p_option = (sidewalk_option_t *)sm->event.ctx;
		if (!p_option) {
			LOG_ERR("sid send msg is NULL");
			break;
		}
		e = sid_option(sm->sid->handle, p_option->option, p_option->data,
			       p_option->data_len);
		if (e) {
			LOG_ERR("sid option err %d", (int)e);
		}
		LOG_DBG("sid option id: %d", (int)p_option->option);
