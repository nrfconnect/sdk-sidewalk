...
#else /* !defined(CONFIG_SIDEWALK_SUBGHZ) */
...
static sid_error_t sid_lib_run(app_context_t *app_ctx)
{
	...
	app_ctx->sidewalk_config.link_config = app_get_ble_config();
	app_ctx->sidewalk_config.time_sync_periodicity_seconds = 7200;
#if defined(CONFIG_SIDEWALK_SUBGHZ)
	app_ctx->sidewalk_config.sub_ghz_link_config = app_get_sub_ghz_config();
#else
...
static void sidewalk_thread(void *context, void *u2, void *u3)
{
	...
	if (SID_ERROR_NONE != application_pal_init()) {
