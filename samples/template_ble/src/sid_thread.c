...
{
	struct sid_thread_ctx *app_ctx = CONTAINER_OF(work, struct sid_thread_ctx, sidewalk_event_work);
...
static void on_sidewalk_event(bool in_isr, void *context)
{
	ARG_UNUSED(in_isr);
	ARG_UNUSED(context);
	if (g_sid_thread_ctx.sidewalk_event_work.handler == NULL) {
		k_work_init(&g_sid_thread_ctx.sidewalk_event_work, sidewalk_event_worker);
	}
	k_work_submit_to_queue(&sidewalk_work_q, &g_sid_thread_ctx.sidewalk_event_work);
}
...
{
	sid_thread_ctx_t *app_ctx = (sid_thread_ctx_t *)context;
...
struct k_work_q * sid_thread_init(void)
{
	...
	#ifdef CONFIG_SIDEWALK_CLI
	CLI_init(&sidewalk_work_q, g_sid_thread_ctx.sidewalk_handle);
	#endif
