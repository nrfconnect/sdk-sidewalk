...
static app_sm_t app_sm = { .app_states = {
				   [STATE_APP_INIT] =
					   SMF_CREATE_STATE(NULL, state_init, NULL, NULL, NULL),
				   [STATE_APP_NOTIFY_CAPABILITY] = SMF_CREATE_STATE(
					   NULL, state_notify_capability, NULL, NULL, NULL),
				   [STATE_APP_NOTIFY_DATA] = SMF_CREATE_STATE(
					   NULL, state_notify_data, NULL, NULL, NULL),
			   } };

static uint8_t __aligned(4)
	app_msgq_buff[CONFIG_SID_END_DEVICE_TX_THREAD_QUEUE_SIZE * sizeof(app_tx_event_t)];
...
static void state_init(void *o)
{
	...
	if (sm->event.call[STATE_APP_INIT] == NULL) {
		LOG_DBG("Can not handle event [%s] without valid handler",
			sm->event.name ? sm->event.name : "UNKNOWN");
		return;
	}
	sm->event.call[STATE_APP_INIT](sm);
}
...
static void state_notify_capability(void *o)
{
	...
	if (sm->event.call[STATE_APP_NOTIFY_CAPABILITY] == NULL) {
		LOG_DBG("Can not handle event [%s] without valid handler",
			sm->event.name ? sm->event.name : "UNKNOWN");
		return;
	}
	sm->event.call[STATE_APP_NOTIFY_CAPABILITY](sm);
}
...
void app_tx_task(void *dummy1, void *dummy2, void *dummy3)
{
	...
	k_msgq_init(&app_sm.msgq, (char *)app_msgq_buff, sizeof(app_tx_event_t),
		    CONFIG_SID_END_DEVICE_TX_THREAD_QUEUE_SIZE);
	smf_set_initial(SMF_CTX(&app_sm), &app_sm.app_states[STATE_APP_INIT]);
...
void app_tx_notify_data_event_notify_sensor(app_sm_t *sm)
{
	...
	if (err) {
		LOG_INF("Temperature get err %d, use dummy value: %d", err, APP_DUMMY_SENSOR_DATA);
...
void app_tx_notify_data_event_notify_button(app_sm_t *sm)
{
	...
	for (size_t i = 0; i < APP_BUTTONS_MAX; i++) {
		if (app_btn_press_mask_bit_is_set(i) && !app_btn_notify_mask_bit_is_set(i)) {
	...
	err = app_tx_demo_msg_send(&state, msg_buffer, &notify_btn_desc, &notify_btn_sid_desc);
	if (err) {
