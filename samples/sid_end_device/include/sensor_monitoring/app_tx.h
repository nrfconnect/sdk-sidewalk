...
#define APP_EVENT_TIME_SYNC_SUCCESS                                                                \
	APP_TX_EVENT(APP_EVENT_TIME_SYNC_SUCCESS,                                                  \
		     {                                                                             \
			     [STATE_APP_INIT] = app_tx_init_event_time_sync_success,               \
		     })
#define APP_EVENT_TIME_SYNC_FAIL                                                                   \
	APP_TX_EVENT(APP_EVENT_TIME_SYNC_FAIL,                                                     \
		     { [STATE_APP_NOTIFY_DATA] = app_tx_notify_data_event_time_sync_fail })
#define APP_EVENT_CAPABILITY_SUCCESS                                                               \
	APP_TX_EVENT(APP_EVENT_CAPABILITY_SUCCESS,                                                 \
		     {                                                                             \
			     [STATE_APP_NOTIFY_CAPABILITY] =                                       \
				     app_tx_notify_capability_event_capability_success,            \
		     })
#define APP_EVENT_NOTIFY_SENSOR                                                                    \
	APP_TX_EVENT(APP_EVENT_NOTIFY_SENSOR,                                                      \
		     { [STATE_APP_NOTIFY_CAPABILITY] =                                             \
			       app_tx_notify_capability_event_notify_sensor,                       \
		       [STATE_APP_NOTIFY_DATA] = app_tx_notify_data_event_notify_sensor })
#define APP_EVENT_NOTIFY_BUTTON                                                                    \
	APP_TX_EVENT(APP_EVENT_NOTIFY_BUTTON,                                                      \
		     { [STATE_APP_NOTIFY_CAPABILITY] = app_tx_notify_capability_not_supported,     \
		       [STATE_APP_NOTIFY_DATA] = app_tx_notify_data_event_notify_button })
#define APP_EVENT_RESP_LED_ON                                                                      \
	APP_TX_EVENT(APP_EVENT_RESP_LED_ON,                                                        \
		     { [STATE_APP_NOTIFY_CAPABILITY] = app_tx_notify_capability_not_supported,     \
		       [STATE_APP_NOTIFY_DATA] = app_tx_notify_data_event_resp_led_on })
#define APP_EVENT_RESP_LED_OFF                                                                     \
	APP_TX_EVENT(APP_EVENT_RESP_LED_OFF,                                                       \
		     { [STATE_APP_NOTIFY_CAPABILITY] = app_tx_notify_capability_not_supported,     \
		       [STATE_APP_NOTIFY_DATA] = app_tx_notify_data_event_resp_led_off })
