...
# Created based on Zephyr https://github.com/zephyrproject-rtos/zephyr/blob/main/scripts/ci/check_compliance.py
...
def get_files(git_filter=None, paths=None):
    filter_arg = (f'--diff-filter={git_filter}',) if git_filter else ()
    ...
    for file in list(files):
        if not os.path.isfile(os.path.join(GIT_TOP, file)):
            ...
            files.remove(file)
    filtered_files = list(filter(check_for_special_files, files))
...
class DevicetreeBindingsCheck(ComplianceTest):
    ...
    def parse_dt_bindings(self):
        ...
        dt_bindings = []
        for file_name in get_files(git_filter="d"):
...
class KconfigBasicCheck(ComplianceTest):
    """
    ...
    def parse_kconfig(self):
        ...
        self.get_modules(os.path.join(
            tempfile.gettempdir(), "Kconfig.modules"))

        # For Kconfig.dts support
        self.get_kconfig_dts(os.path.join(
            tempfile.gettempdir(), "Kconfig.dts"))
...
class Nits(ComplianceTest):
    ...
    def run(self):
        ...
        for fname in get_files(git_filter="d"):
            if "Kconfig" in fname:
...
class BinaryFiles(ComplianceTest):
    ...
    def run(self):
        BINARY_ALLOW_PATHS = ("doc/", "lib/", "samples/")
...
class ImageSize(ComplianceTest):
    ...
    def run(self):
        ...
        for file in get_files(git_filter="d"):
    ...
    def run(self):
        ...
        for file in get_files(git_filter="d"):
            ...
            if file.startswith(".github/"):
                ...
                yaml_config.rules["truthy"]["allowed-values"].extend([
                                                                     'on', 'off'])
