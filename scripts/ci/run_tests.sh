...
echo "> Build samples"
$TWISTER_BIN --verbose -n --build-only --show-footprint --testcase-root $SIDEWALK_BASE/samples

echo "> Run unit tests"
$TWISTER_BIN --verbose -n --platform native_posix --coverage --enable-ubsan --enable-lsan --enable-asan  --testcase-root $SIDEWALK_BASE/tests/unit_tests
...
if [ -f "$HARDWARE_MAP_FILE" ]; then
	$TWISTER_BIN --verbose -n --testcase-root $SIDEWALK_BASE/tests/functional --device-testing --hardware-map $HARDWARE_MAP_FILE
else
