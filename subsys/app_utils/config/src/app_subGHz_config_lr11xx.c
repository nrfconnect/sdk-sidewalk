...
const void *get_radio_cfg(void)
{
	...
	radio_lr11xx_cfg.gpios.power = sid_gpio_utils_register_gpio(
		(struct gpio_dt_spec)GPIO_DT_SPEC_GET_OR(LORA_DT, reset_gpios, INVALID_DT_GPIO));
	radio_lr11xx_cfg.gpios.int1 = sid_gpio_utils_register_gpio(
		(struct gpio_dt_spec)GPIO_DT_SPEC_GET_OR(LORA_DT, event_gpios, INVALID_DT_GPIO));
	radio_lr11xx_cfg.gpios.radio_busy = sid_gpio_utils_register_gpio(
		(struct gpio_dt_spec)GPIO_DT_SPEC_GET_OR(LORA_DT, busy_gpios, INVALID_DT_GPIO));
	...
	radio_lr11xx_cfg.bus_selector.client_selector = sid_gpio_utils_register_gpio(
		(struct gpio_dt_spec)SPI_CS_GPIOS_DT_SPEC_GET(LORA_DT));
