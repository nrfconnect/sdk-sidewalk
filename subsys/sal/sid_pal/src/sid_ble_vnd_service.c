...
static void vnd_srv_notif_changed(const struct bt_gatt_attr *attr, uint16_t value);
static ssize_t vnd_srv_on_write(struct bt_conn *conn, const struct bt_gatt_attr *attr,
				const void *buf, uint16_t len, uint16_t offset, uint8_t flags);

/* VENDOR_SERVICE definition */
BT_GATT_SERVICE_DEFINE(vnd_service, BT_GATT_PRIMARY_SERVICE(VND_SID_BT_UUID_SERVICE),
		       BT_GATT_CHARACTERISTIC(VND_SID_BT_CHARACTERISTIC_WRITE,
					      BT_GATT_CHRC_WRITE_WITHOUT_RESP, BT_GATT_PERM_WRITE,
					      NULL, vnd_srv_on_write, NULL),
		       BT_GATT_CHARACTERISTIC(VND_SID_BT_CHARACTERISTIC_NOTIFY, BT_GATT_CHRC_NOTIFY,
...
static void vnd_srv_notif_changed(const struct bt_gatt_attr *attr, uint16_t value)
{
	...
	ARG_UNUSED(attr);
	LOG_DBG("Notification for VENDOR_SERVICE is %s.", notif_enabled ? "enabled" : "disabled");
